...
- [x] Docker-Setup (`Dockerfile` mit Non-Root User `appuser`, Python 3.11-slim, ffmpeg) & `docker-compose.yml` (PostgreSQL 16 Alpine)
...
- [x] GitHub Ecosystem (`.github/` mit CI, Docker-Build, CodeQL, Dependabot, Issue/PR-Templates & Verifikationsskripten)
...
3. **Flexible Datenbankanbindung:** Standardmäßig über isolierten PostgreSQL 16 Container in `docker-compose.yml`, nahtlos umschaltbar auf externe PostgreSQL-Instanzen über `DATABASE_URL`.
